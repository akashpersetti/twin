...
import { Github, FolderGit2, Globe } from 'lucide-react';
...
function ProjectLinks({ project }: { project: Project }) {
  ...
  return (
    <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
      {githubUrl && <IconLinkButton href={githubUrl} label={`${project.title} on GitHub`} icon={Github} />}
      {liveUrl && <IconLinkButton href={liveUrl} label={`${project.title} live link`} icon={Globe} />}
    </div>
  );
}
...
function GridProject({ project, index, onClick }: {
  project: Project; index: number; onClick: () => void;
}) {
  return (
    <div
      onClick={onClick}
      className="cursor-pointer border-t pt-5 pb-5"
      style={{ borderColor: 'var(--border)' }}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <span className="mono text-[11px] tabular-nums" style={{ color: 'var(--text-faint)' }}>
...
export default function Projects() {
  ...
  const featured = resume.projects[featuredIndex];
  const rest = resume.projects
      ...
      <div className="max-w-5xl mx-auto section-border">
        ...
        <div className="mt-10 flex justify-center">
          <a
            ...
            className="hover-accent group inline-flex items-center gap-2 px-6 py-2.5 text-sm font-medium transition-colors"
